...
| Network-binding sidecar relabeling (**accepted limitation**, bot-strategy#942) | `<journal>.network-binding.json` is written once by `prepare` beside the journal, outside the journal's hash chain; `submit`/`reconcile` only compare it with a binding re-derived from the *current* config. Two principals, assessed separately. A **sidecar-only writer** (the trading user, which owns the journal directory) can relabel a historical journal's network/routing context for `prepare`'s residual aggregation, or make `submit`/`reconcile` refuse the current journal (fail-closed); it cannot redirect a submission, and the journal's own content stays under its protected head. A **sidecar-and-config writer** (root: `/etc` is read-only to the trading user) can also make the two agree and defeat the prepare→submit drift detection the sidecar exists for, e.g. route a prepared IOC to a different `vaultAddress` — that access already reaches the signer and routing configuration, so this adds no privilege. Not prevented for either, on one account and one network | The aggregation's live-balance bound still rejects any residual total that exceeds holdings. Reopen before a second network or account context is ever configured, or if the journal directory and the configuration ever share a writer by design |
