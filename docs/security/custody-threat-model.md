...
| Network-binding sidecar relabeling (**accepted limitation**, bot-strategy#942) | `<journal>.network-binding.json` is written once by `prepare` outside the journal's hash chain; a host principal that can rewrite it could relabel a journal's network/routing context for aggregation. Not prevented: the same write access already reaches the operational config beside it, and the deployment is one account on one network | `submit`/`reconcile` re-derive and compare the binding from their own config, so a relabeled sidecar only ever affects which historical journals `prepare` admits into residual aggregation; the live-balance bound rejects any total that exceeds holdings. Reopen before a second network or account context is ever configured |
