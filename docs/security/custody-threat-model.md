...
| Network-binding sidecar relabeling (**accepted limitation**, bot-strategy#942) | `<journal>.network-binding.json` is written once by `prepare` beside the journal, outside the journal's hash chain; `submit`/`reconcile` only compare it with a binding re-derived from the *current* config and operational file. The sidecar is accepted at the strength of its neighbours, not on its own. The **journal-directory owner** (the trading user the live unit runs as) can rewrite the sidecar, but can equally rewrite a journal together with its colocated `<journal>.protected-head.json` (a consistent forged pair passes the per-journal check) and the operational file under `/var/lib/hype-accumulator/` that `is_mainnet` is read from (the service writes the history binding beside it): it can relabel or forge a historical residual for `prepare`'s aggregation and pass the network half of the prepare→submit drift check without root. It cannot change `/etc` (endpoint, security policy, `execution_account_kind`), so a submission still goes to the configured endpoint under the configured `vaultAddress` routing, and the same user already holds the signer in its process. A **configuration writer** (root) can additionally change routing and reaches the signer material at rest. Not prevented for either, on one account and one network | The aggregation's live-balance bound still rejects any residual total that exceeds holdings. Reopen before a second network or account context is ever configured, or if a principal ever exists that can write the sidecar but not the journal, its head and the operational file beside it |
