...
| Network-binding sidecar relabeling (**accepted limitation**, bot-strategy#942) | `<journal>.network-binding.json` is written once by `prepare` outside the journal's hash chain; `submit`/`reconcile` only compare it with a binding re-derived from the *current* config. A host principal that can write the sidecar can relabel a journal's network/routing context for aggregation; one that can also write the config beside it (the same access, on this host) can defeat the prepare→submit drift detection the sidecar exists for, e.g. route a prepared IOC to a different `vaultAddress`. Not prevented: that write access already reaches the signer and routing configuration, so this is no additional privilege, and the deployment is one account on one network | The aggregation's live-balance bound still rejects any residual total that exceeds holdings. Reopen before a second network or account context is ever configured |
